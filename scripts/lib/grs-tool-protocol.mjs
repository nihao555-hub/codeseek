...
export const TOOL_CONTINUE_HINT = [
  ...
  'CRM: mcp__trade-crm__search_queries, upsert_lead, draft_outreach, quote_catalog, record_reply, upsert_deal. Outreach is draft-only. quote_catalog reads store/data/catalog.json (hints like tumbler map to HK-TB-500-SS). record_reply is for the NAMED buyer qty, not other leads.',
...
export function buildToolProtocolPrompt(tools) {
  ...
  return [
    ...
    '- CRM: mcp__trade-crm__search_queries then web_search; upsert_lead with a public URL; draft_outreach is not sent; quote_catalog reads store/data/catalog.json; record_reply updates the named buyer and writes team/deals/<id>.md',
