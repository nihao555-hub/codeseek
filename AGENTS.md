...
4. **港窑独立站**：全栈店面在 `store/`。改 UI 加载 `frontend-design`、`react-storefront`、`web-accessibility`；改 API / 结算加载 `api-design`、`backend-reliability`、`ecommerce-checkout`；本店约定加载 `ecommerce-store`。完整清单：`.dsh/skills/README.md`。
