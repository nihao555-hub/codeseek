...
对齐网易外贸通「运营专家」：分组、沉睡激活、动态监测。本仓库没有海关库，线索只能来自：

- `web_search` / 用户粘贴的公开网页（必须写来源 URL）
- 独立站询盘表单
- 用户自己提供的名片或邮件

分组：`hot`（本周可触达） / `warm`（已互动） / `nurture`（长期） / `sleeping`（>30 天无回复） / `closed`。

没有公开邮箱就写 `TBD — 不要编`。触达状态只允许：`draft`（已出稿未发） / `user-sent`（用户说已发出） / `replied` / `none`。
