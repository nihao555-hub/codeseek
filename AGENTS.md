...
联网搜索优先 `mcp__web-search__web_search`（SearXNG，失败则 DuckDuckGo）。不要调用原生 `web_search`。没有该工具时再用 bash/`curl`，并标明不确定之处。装配 MCP / 远程 skill 用 `assemble-toolkit`（`npm run toolkit`）。
